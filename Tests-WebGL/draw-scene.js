...
drawDots = function() {
    ...
    for (let i = 0; i < 15000; i += 1) {
        m = sin(t2) * 750;
        n = map(sin(t2), -1, 1, 0.5, 0.75);
        x = cos(i * 20 + fx * 0.25 + t) * i * 0.0005;
        y = sin(i * 20 + fx * 0.25 + t) * i * 0.0005;
//         x += sin(x * sin(x) * 0.25);
//         y += cos(y * sin(x) * 0.25);
//         x *= pow(x, 3);
//         x = sin(x + y);
        fx = sin(x) * tan(t2 * 0.25 + fx * 0.05 + i * 0.0001);
//         fy = y + fx * (sin(t2 + fx) + 0.003);
//         x += (Math.random() - 0.5) * 0.00005;
//         y += (Math.random() - 0.5) * 0.00005;
        x += xOffset * 0.25;
        y += yOffset * 0.25;
        vertices.push(x * 0.125 * 1, y * 0.2 * 1, 0.0);
    }
