var x, y, t, w;
var drawCount = 0;
var looping = true;
var showPanel = true;
var speed = 0;
var accMult = 0;
var amounts = 1;
var shape = "circle";
var showYellow = true;
var norma = false;
var formattedXVel = 0;
var formattedYVel = 0;
var formattedXAcc = 0;
var formattedYAcc = 0;
var drawUpdate = 0;

function setup() {
    createCanvas(windowWidth,windowHeight);
    frameRate(30);
    createInfoDiv();
    setupInfoDiv();
    translate(width/2, height/2);
    if(!looping){noLoop();}
    noStroke();
    w = new Walker();
}

function draw() {
    if (drawUpdate > 5) {
        sliderInfo6.html("White dot vel. x: " + formattedXVel + ", y: " + formattedYVel);
        sliderInfo7.html("White dot acc. x: " + formattedXAcc + ", y: " + formattedYAcc);
        
        drawUpdate = 0;
    }
    background(0,10);
    scale(zoom, zoom);
    for (var i = 0; i < amounts; i++) {
        t = drawCount * speed;
        var v = createPath(t);
        w.update(v);
        fill(255);
        w.display();
        if (showYellow) {
            fill(255,255,0);
            ellipse(v.x, v.y, 2.5, 2.5);
        }
        drawCount++;
    }
    drawUpdate++;
}

function createPath(tt) {
    var x, y;
    switch(shape) {
        case "circle" : 
            x = sin(tt) * 100;
            y = cos(tt) * 100;
        break;
        case "top" :
            x = pow(sin(tt),11) * 100;
            y = cos(tt) * 100;
        break;
        case "superellipse" :
            var ct = cos(tt);
            var st = sin(tt);
            x = 4 * Math.sign(ct) * pow(abs(ct), 2/4) * 40 ;
            y = 4 * Math.sign(st) * pow(abs(st), 2/4) * 25 ;
        break;
        case "lissajous" :
            x = sin(tt*2) * 100;
            y = cos(tt*3) * 100;
        break;
        default : 
            x = sin(tt) * 100;
            y = cos(tt) * 100;
    }
    var v = createVector(x, y);
    return v;
}

function Walker()  {
    this.pos = createVector(0, 0);
    this.vel = createVector(0, 0);

    this.reset = function() {
        this.pos = createVector(0,0);
        this.vel = createVector(0, 0);
    }

    this.update = function(vec)  {
        this.acc = p5.Vector.sub(vec, this.pos);
        if (norma) {this.acc.normalize();}
        this.acc.mult(accMult);
        formattedXAcc = Math.round(this.acc.x*100)/100;
        formattedYAcc = Math.round(this.acc.y*100)/100;
        // this.acc.setMag(accMult);
        this.vel.add(this.acc);
        formattedXVel = Math.round(this.vel.x*100)/100;
        formattedYVel = Math.round(this.vel.y*100)/100;

        this.pos.add(this.vel);
    }

    this.display = function() {
        ellipse(this.pos.x, this.pos.y, 2.5, 2.5);
    }
}

function keyPressed() {
    if (keyCode === 32) {
        if (looping) {noLoop(); looping = false;} else {loop(); looping = true;}
    }
    if (key == 'y' || key == 'Y')  {
        showYellow = (showYellow) ? false : true;
    }

    if (key == 't' || key == 'T') {
        if (showPanel) {
            showPanel = false;
            infoDiv.style("display","none");
        } else {
            showPanel = true;
            infoDiv.style("display","block");
        }
    }
}