        ...
        for (let x = 0; x < face[0].length; x++) {
         for (let y = 0; y < face.length; y++) {
            let p = face[y][x];
             ...
             if (p == "1") {
                 vertices.push(x * 0.0025 * sc * (9 / 16) * 4 + tx, -y * 2.2 * sc * 0.01 - 0.15 + Math.sin(drawCount * 0.25 + y * 0.75) * 0.005 + ty - 0.02, 20.0 * sc * 0.9, 1);
                             num++;
                colors.push(0, 0, 0);                 
             } else if (p == "0") {
                 vertices.push(x * 0.0025 * sc * (9 / 16) * 4 + tx, -y * 2.2 * sc * 0.01 - 0.15 + Math.sin(drawCount * 0.25 + y * 0.75) * 0.005 + ty - 0.02, 20.0 * sc * 0.9, 1);
...
roundedSquare.fragText = `
    ...
    void main(void) {
         ...
         vec2 uv = gl_PointCoord.xy;
        uv = uv * 2. - 1.;
        float color = roundedRectangleFlicker(uv, vec2(0.0, 0.0), vec2(0.75, 0.35) * 1.5, 0.08, 0.3);
...
TerminalRecorder.prototype.stop = function() {
    ...
    document.onkeydown = keyDown;
            ...
            ".0000000000000000000001222000000000022.",
            "..00000011000000001111122000000011222..",
