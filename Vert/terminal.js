...
roundedSquare.fragText = `
    ...
    void main(void) {
         ...
         vec2 uv = gl_PointCoord.xy;
        uv = uv * 2. - 1.;
        float color = roundedRectangleFlicker(uv, vec2(0.0, 0.0), vec2(0.125, 0.21) * 5., 0.002, 0.025);
        float rando = rand(uv * time) * 0.05;
       float vig = (roundedRectangle(gl_FragCoord.xy, vec2(0.0, 580.0), vec2(2.825, 0.915) * 587.2, 1.05, 350.5) + 0.0);
