...
void main() {
   ...
   gl_FragColor.rgb = (gl_FragColor.rgb - (rando * 0.075)) * 1.5;
}
`;
textureShader.init();
...
// 
    void main(void) {
        float t = time * 0.125e-2;
        float osc = map(sin(t * 16e-1), -1., 1., 0.05, 4.005);
        float i = vertexID * 1e-1;
        float x =cos((i * 0.5 * sin(i * 1e2 * 0.2e-5) * 0.00105)) * pow(i, 5.) * 1e-16;
        float y =sin((i * 0.5 * sin(i * 1e2 * 0.2e-5) * 0.00105)) * pow(i, 5.) * 1e-16;
        // x += cos(tan(tan(i * 500e5 * i * i))) * 0.6;
        // y += cos(tan(tan(i * 500e5 * i * i))) * 0.6;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        ...
        x *= cos(sin(distance * 1e1 + t * 0.5e1 + 1000. * 0.5e3) * 0.125 + 1. + cos(distance * 100.1) * 2e4 * 1e-5);
        y *= sin(sin(distance * 1e1 + t * 0.5e1 + 1000. * 0.5e3) * 0.125 + 1. + cos(distance * 100.1) * 2e4 * 1e-5);
        // distance = sqrt(pow(x, 2.) + pow(y, 2.));
        // x *= cos((distance * 0.00001) * 2e1);
        // y *= sin((distance * 0.00001) * 2e1);
        // x += cos(t * 0.25e1 + i * 1e-2) * i * 0.0025;
        // y += sin(t * 0.25e1 + i * 1e-2) * i * 0.0025;
        // x += cos(t * 0.25e2) * i * 0.05;
        // y += sin(t * 0.25e2) * i * 0.05;
        //         float x = cos(i) * i * 1e-5 * 2.;
//         float y = sin(i) * i * 1e-5 * 2.;
        gl_Position = vec4(x * 3.67 * 0.0625 * 0.0625e-3 - 0.49, y * 0.21 * 0.0625e-3 - 0.5, 0.0, 1.0);
//         center = vec2(gl_Position.x, gl_Position.y);
//         center = 512.0 + center * 512.0;
//         myposition = vec2(gl_Position.x, gl_Position.y);
        alph = 0.25 * 0.5;
        gl_PointSize = (1.5 + min((distance * 0.15 * 0.125 * 0.125 * 0.125), 4.5e1)) * 1.5;
        // gl_PointSize = 12.;
        // gl_PointSize = 25.0 + cos((coordinates.x + coordinates.y) * 4000000.) * 5.;
        // gl_PointSize = coordinates.z / (alph * (sin(myposition.x * myposition.y * 1.) * 3. + 0.5));
    }
