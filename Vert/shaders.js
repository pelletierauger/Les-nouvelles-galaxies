...
// 
    void main(void) {
        ...
        float x =cos((i * 0.5 * sin(i * 0.2e-5) * 0.001)) * pow(i, 5.) * 1e-16;
        float y =sin((i * 0.5 * sin(i * 0.2e-5) * 0.001)) * pow(i, 5.) * 1e-16;
        x += cos(tan(i * 1. * tan(i * 500e5 * i * i))) * 2500.6;
        y += cos(tan(i * 1. * tan(i * 500e5 * i * i))) * 2500.6;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        ...
        // x += cos(152. * 0.5e-1 * t + cos(distance) * 0.5) * i * 0.25;
        // y += sin(152. * 0.5e-1 * t + sin(distance) * 0.5) * i * 0.25;
        //         float x = cos(i) * i * 1e-5 * 2.;
//         float y = sin(i) * i * 1e-5 * 2.;
        gl_Position = vec4(y * 1.67 * 0.0625 * 0.15e-3 - 0.49, x * 0.19 * 0.15e-3 - 0.5, 0.0, 1.0);
//         center = vec2(gl_Position.x, gl_Position.y);
//         center = 512.0 + center * 512.0;
//         myposition = vec2(gl_Position.x, gl_Position.y);
        alph = 0.25 * 0.5;
        gl_PointSize = (1.5 + min((distance * 0.15 * 0.125 * 0.125 * 0.2), 4e4)) * 1.5;
        // gl_PointSize = 12.;
        // gl_PointSize = 25.0 + cos((coordinates.x + coordinates.y) * 4000000.) * 5.;
        // gl_PointSize = coordinates.z / (alph * (sin(myposition.x * myposition.y * 1.) * 3. + 0.5));
    }
