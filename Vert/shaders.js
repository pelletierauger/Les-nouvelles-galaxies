...
// 
    void main(void) {
        ...
        float bx = cos(2.5e-1 * i);
        float by = sin(2.5e-1 * i);
        float x = pow(bx, sin(i) * 1e2) * i * 2e-3;
        float y = pow(by, sin(i) * 1e2) * i * 2e-3;
        x += sin(cos(i * 4e2) * bx * 1e4 + cos(t * 4e1)) * 3400.5 * bx;
        y += sin(sin(i * 4e2) * by * 1e4 + sin(t * 4e1)) * 3400.5 * by;
        // x += sin(cos(i * 4e2) * i * 1e-4) * 40000.;
        // y += sin(sin(i * 4e2) * i * 1e-4) * 40000.;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        x += cos(t * 16.75e1) * i * 0.0005;
        y += sin(t * 16.75e1) * i * 0.0005;
...
//         float y = sin(i) * i * 1e-5 * 2.;
        gl_Position = vec4(x * 0.2 * 2e-1, y * 0.33e-1 * 2., 0.0, 1.0);
//         center = vec2(gl_Position.x, gl_Position.y);
//         center = 512.0 + center * 512.0;
//         myposition = vec2(gl_Position.x, gl_Position.y);
        alph = 0.25 * 0.5;
...
//     varying vec2 center;
    ...
    void main(void) {
        ...
        float rando = rand(pos);
        // gl_FragColor = vec4(1.0, (1.0 - dist_squared * 40.) * 0.6, 0.0, alpha + ((0.12 - dist_squared) * 4.) - (rando * 0.2));
        gl_FragColor = vec4(1.0, 0.4 - dist_squared, 0.0 + alpha * 120., ((3. - dist_squared * 24.0 * (0.25 + alph) - (rando * 1.1)) * 0.045 + alpha)) * 1.5;
//         gl_FragColor = gl_FragColor.gbga;
        gl_FragColor.rgb = vec3(1.) - gl_FragColor.rgb;
             gl_FragColor = gl_FragColor.grba;
//         gl_FragColor.g *= 0.525;
        gl_FragColor.b *= 0.5;
        gl_FragColor.rgb = gl_FragColor.rrr * 1.1;
