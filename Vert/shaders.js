...
//         y *= 0.25 * 44.5;
        ...
        x *= cos(sin(distance * 1e-2 + t * 0.5e3) * 0.125 + t * 1e1 + cos(distance * 0.1) * 2e4 * 1e-5);
        y *= sin(sin(distance * 1e-2 + t * 0.5e3) * 0.125 + t * 1e1 + cos(distance * 0.1) * 2e4 * 1e-5);
...
//         myposition = vec2(gl_Position.x, gl_Position.y);
        alph = 0.25 * 0.5;
        gl_PointSize = (1.5 + min((distance * 0.15 * 0.125 * 0.125 * 0.125), 1.5e1)) * 1.5;
        // gl_PointSize = 12.;
        // gl_PointSize = 25.0 + cos((coordinates.x + coordinates.y) * 4000000.) * 5.;
        // gl_PointSize = coordinates.z / (alph * (sin(myposition.x * myposition.y * 1.) * 3. + 0.5));
    }
