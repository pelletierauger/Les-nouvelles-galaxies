...
// 
    void main(void) {
        ...
        float i = vertexID;
        float x = tan(pow(i, 3.5) / cos(i * 1e-2)) * i * 0.25e-4;
        float y = tan(pow(i, 3.5) / sin(i * 1e-2)) * i * 0.25e-4;
        // x = tan(x * 10.5) * 1e-3 + x * 0.5;
        // y = tan(y * 10.5) * 1e-3 + y * 0.5;
        // x = tan((x + y));
...
//         float y = sin(i) * i * 1e-5 * 2.;
        gl_Position = vec4(x * 1. * 0.05, y * 0.05, 0.0, 1.0);
//         center = vec2(gl_Position.x, gl_Position.y);
//         center = 512.0 + center * 512.0;
//         myposition = vec2(gl_Position.x, gl_Position.y);
        alph = 0.25 * 0.5;
