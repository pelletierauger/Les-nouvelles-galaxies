...
// 
    void main(void) {
        ...
        float i = vertexID;
        float x = cos(tan(tan(i * 4e-5 * t * 1e2) * 1e36) + cos(i + t * 1e2) * 5e-2) * i * 16e-6;
        float y = sin(tan(tan(i * 4e-5 * t * 1e2) * 1e36) + sin(i + t * 1e2) * 5e-2) * i * 16e-6;
//         x = tan(x * 0.225) + x * 0.2;
//         y = tan(y * 0.225) + y * 0.2;
        x *= 0.25 * 2.;
        y *= 0.25 * 2.;
...
//         float y = sin(i) * i * 1e-5 * 2.;
        x += cos(t * 8.75) * i * 2.5e-6;
        y += sin(t * 8.75) * i * 2.5e-6;
//         x *= (sin(t * 1e1) + 1.5) * 10.;
//         y *= (sin(t * 1e1) + 1.5) * 10.;
        gl_Position = vec4(x * 0.6 * 0.75, y * 0.75, 0.0, 1.0);
//         center = vec2(gl_Position.x, gl_Position.y);
//         center = 512.0 + center * 512.0;
//         myposition = vec2(gl_Position.x, gl_Position.y);
