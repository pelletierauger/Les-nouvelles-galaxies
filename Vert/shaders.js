...
//     col = mix(mountCol * 0.5, col, mountain2);
    ...
    float rando = rand(vec2(uv.x, uv.y) * 100.);
//     col *= 0.2 + 0.8 * pow(32.0 * uv.x * uv.y * (1.0 - uv.x) * (1.0 - uv.y), 0.2);
    gl_FragColor = vec4(vec3(1.0) - rando * 0.1, 1.0);
//     gl_FragColor.b *= 0.25;
//     gl_FragColor = gl_FragColor.brga;
//     gl_FragColor.r = gl_FragColor.r - rando * 0.1;
    // gl_FragColor = gl_FragColor.grba;
    ...
    gl_FragColor.b = sin(2. + uv.y * 4.) * 0.25;
        gl_FragColor.r = sin(2. + uv.y * 4. * cos(uv.x + sin(uv.y * 1e4) * sin(uv.y * 100. + t * 1e1))) * 0.5;
    gl_FragColor.r += sin(pow(uv.x, 4.) * uv.x);
    // gl_FragColor.r += cos(pow(uv.y, 2.) * uv.y) * 0.1;
    gl_FragColor = gl_FragColor.rrra - rando * 0.125;
...
// 
    void main(void) {
        ...
        float x = cos(pow(i, sin(i * 2e4)) * 20e1 * cos(t)) * i * 2e-2;
        float y = sin(pow(i, sin(i * 2e4)) * 20e1 * sin(t)) * i * 2e-2;
        x += tan(x * 1e-3 + cos(i * 1e1 * tan(x)) + t * 1e2) * 500.5;
        y += tan(x * 1e-3 + sin(i * 1e1 * tan(x)) + t * 1e2) * 500.5;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        // x += cos(t * 2.75e1) * i * 0.005;
        // y += sin(t * 2.75e1) * i * 0.005;
        // x += 200. * cos(t * 16.);
        // y += 200. * sin(t * 16.);
        x *= 0.5e-3;
        y *= 0.5e-3;
...
//     varying vec2 center;
    ...
    void main(void) {
        ...
        float rando = rand(pos);
        // gl_FragColor = vec4(1.0, (1.0 - dist_squared * 40.) * 0.6, 0.0, alpha + ((0.12 - dist_squared) * 4.) - (rando * 0.2));
        gl_FragColor = vec4(1.0, 0.4 - dist_squared, 0.0 + alpha * 120., ((3. - dist_squared * 24.0 * (0.25 + alph) - (rando * 1.1)) * 0.045 + alpha)) * 1.2;
